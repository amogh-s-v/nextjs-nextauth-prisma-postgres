...
import { createTheme, ThemeProvider, Box, Typography, Button, Modal, Paper, Table, TableBody, TableCell, TableHead, TableRow, styled, TableContainer, tableCellClasses } from '@mui/material';
...
const SentimentHistory = () => {
  ...
  return (
            ...
            <TableBody>
              {sentimentHistory.map((sentiment, index) => (
                <StyledTableRow key={index}>
                  <StyledTableCell>{formatDate(sentiment.date)}</StyledTableCell>
                  <StyledTableCell>{sentiment.sentiment}</StyledTableCell>
                  <StyledTableCell>
                    <Button startIcon={<VisibilityIcon />} onClick={() => handleOpenModal(sentiment.journal_text)}>
                      Show Entry
                    </Button>
                  </StyledTableCell>
                </StyledTableRow>
    ...
    <div style={{ backgroundColor: 'white', padding: '20px', borderRadius: '5px', position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)' }}>
